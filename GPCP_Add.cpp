...
arma::uvec sampleInt(arma::uword nCross, arma::uword nInd){
  arma::uvec samples;
  samples.set_size(nCross);
  if(nCross == 0){
    return samples;
  }
  double q, v, x, y1, y2;
  arma::uword threshold = 13*nCross;
  arma::uword S, limit, top, bottom;
  arma::vec u(1,arma::fill::randu);
  v = exp(log(u(0))/double(nCross));
  q = double(nInd-nCross+1);
  while((nCross>1) & (threshold<nInd)){
    while(true){
      while(true){
        x = double(nInd)*(1-v);
        S = floor(x);
      ...
      u.randu();
      y1 = exp(log(u(0)*double(nInd)/q)/double(nCross-1));
      v = y1*(1-x/double(nInd))*(q/(q-double(S)));
      if(v <= 1){
        break;
      }
      y2 = 1;
      ...
      for(arma::uword i=nInd-1; i>=limit; --i)
        y2 *= double(top)/double(bottom);
      u.randu();
      if((double(nInd)/(double(nInd)-x)) >= (y1*exp(log(y2)/double(nCross-1)))){
        v = exp(log(u(0))/double(nCross-1));
        break;
      }
      v = exp(log(u(0))/double(nCross));
    }
    samples(nCross-1) = S+1;
    nInd = nInd-S-1;
    --nCross;
    q = double(nInd-nCross+1);
    threshold -= 13;
  ...
  if(nCross > 1){
    ...
    while(nCross >= 2){
      u.randu();
      S = 0;
      q = double(top)/double(nInd);
      while(q > u(0)){
        ++S;
        --top;
        --nInd;
        q = (q*double(top))/double(nInd);
      }
      samples(nCross-1) = S+1;
      --nInd;
      --nCross;
    ...
    u.randu();
    samples(0) = floor(u(0)*nInd);
  }else{
    samples(0) = floor(v*nInd);
  }
...
arma::vec calcContr(arma::uvec crosses, const arma::uword& nInd,
                    arma::uvec& rowPos, arma::uvec& colPos){
...
void calcVec(double& angle, double& length,
             double u, double sim,
             double uMax, double simMax,
             double uMin, double simMin){
  u = (u-uMin)/(uMax-uMin);
...
// Note: This implementation is slow and could be greatly improved
...
Rcpp::List selectCrossPlan(arma::uword cycleNumber,
  ...
  M -= 1.0;
  arma::mat G = (M*M.t())/M.n_cols + 1.0;
  
  // Calculate mean progeny performance including additive and dominance effects
  M = M.t()+1;
  arma::vec u(potCross);
#pragma omp parallel for schedule(static) 
  for(arma::uword i=0; i<potCross; ++i){
    arma::vec p1 = M.col(rowPos(i));                 //.col = extract column vector; allele dosage of parent 1
    arma::vec p2 = M.col(colPos(i));                 // allele dosage of parent 2
    arma::vec pA = (p1+p2)/2;                        // parent average allele dosage
    // u(i) = accu(a%(2*p-y+1)+d%(2*p%(1-p)+y%(2*p-1)));    // Predicted average progeny performance - % = element-wise multiplication in armadillo
    u(i) = accu(a%pA);    // Parent average (does not include heterotic component)
  }
