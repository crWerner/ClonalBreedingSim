...
if (dominance_degree > 0) {  # see "0_GlobalParameters.R"
  ...
  seed_output$rEBV[b] <- cor(seedlings@pheno, bv(seedlings))
  seed_output$rEGV[b] <- cor(seedlings@pheno, seedlings@gv)
  ...
  st1_output$rEGV[b] <- cor(ST1@pheno, ST1@gv)
  ...
  seed_output$rEBV[b] <- cor(seedlings@pheno, seedlings@gv)
  st1_output$rEBV[b] <- cor(ST1@pheno, ST1@gv)
